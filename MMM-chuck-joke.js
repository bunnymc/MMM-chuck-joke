 ...
 * MIT Licensed.
 */
Module.register("MMM-chuck-joke",{
...
"Chuck Norris\' hand is the only hand that can beat a Royal Flush"
	...
	randomIdx: function(jokes) {
		...
		var jokeIdx = generate();

		while (jokeIdx === this.lastJokeIdx) {
			jokeIdx = generate();
		}

		this.lastJokeIdx = jokeIdx;

		return jokeIdx;
	},

	randomJoke: function() {
		var idx = this.randomIdx(this.config.jokes);
		return this.config.jokes[idx];
	},

	getDom: function() {
		var jokeTxt = this.randomJoke();
		var wrapper = document.createElement("span");
		if (this.data.position.indexOf("left") >= 0) {
                    wrapper.className = "left";
                } else {
                    if (this.data.position.indexOf("right") >= 0) {
                        wrapper.className = "right";
                    } else {
                        if (this.data.position.indexOf("center") >= 0) {
